/**
 * One-shot newsletter campaign: Gumbo Cherries Indoor drop.
 *
...
const PRODUCT_SLUG = "gumbo-cherries-indoor";
const CAMPAIGN_SLUG = "gumbo-cherries-newsletter-2026";
...
function buildEmailHtml(product: Product, recipientEmail: string): string {
  ...
  h += '<tr><td style="padding:0 0 14px;">';
  h += '<p style="margin:0;font-size:16px;line-height:1.6;color:#1a1a1a;"><strong>Gumbo Cherries Indoor</strong> just dropped — sweet bubble-gum nose, cherry-front terps, indoor cure with full trichome density. Cleanest Gumbo pheno we\'ve seen this year.</p>';
  h += "</td></tr>";

  // ── HERO IMAGE 1 ──
  h += '<tr><td style="padding:4px 0 4px;">';
  h += `<a href="${productUrl}" style="display:block;text-decoration:none;border:0;outline:none;"><img src="${esc(heroImg)}" alt="Gumbo Cherries Indoor" width="580" style="display:block;width:100%;max-width:580px;height:auto;border:0;outline:none;border-radius:8px;"/></a>`;
  h += "</td></tr>";
  ...
  h += '<p style="margin:6px 0 0;font-size:15px;line-height:1.55;color:#7c2d12;">Use code <strong style="background:#fed7aa;padding:2px 8px;border-radius:4px;font-family:Menlo,Consolas,monospace;color:#7c2d12;">CHERRIES15</strong> at checkout. Saves you ' + esc(`$${Math.round((parseFloat(live.replace(/[^\d.]/g, "")) || 500) * 0.15)}`) + ' on a single-pound order. Valid 72 hours.</p>';
  h += '</td></tr></table>';
  h += "</td></tr>";
  ...
  h += '<tr><td style="padding:4px 0;font-size:15px;line-height:1.55;color:#333333;">✓ <strong style="color:#1a1a1a;">Cherry-bubblegum flavor profile</strong> — distinctive, not generic.</td></tr>';
  h += '<tr><td style="padding:4px 0;font-size:15px;line-height:1.55;color:#333333;">✓ <strong style="color:#1a1a1a;">10/10 trichome density</strong> — full indoor cure, frosted bag-appeal.</td></tr>';
  ...
  h += '<tr><td style="padding:4px 0 4px;">';
  h += `<a href="${productUrl}" style="display:block;text-decoration:none;border:0;outline:none;"><img src="${esc(detailImg)}" alt="Gumbo Cherries Indoor — bud detail" width="580" style="display:block;width:100%;max-width:580px;height:auto;border:0;outline:none;border-radius:8px;"/></a>`;
  h += "</td></tr>";

  // ── Urgency + Final CTA ──
  h += '<tr><td style="padding:22px 0 8px;">';
  h += '<p style="margin:0;font-size:15px;line-height:1.6;color:#1a1a1a;">These move in 24–48 hours. Code <strong>CHERRIES15</strong> expires in 72 hours.</p>';
  h += "</td></tr>";
...
function buildEmailText(product: Product, _recipientEmail: string): string {
  ...
  return [
    `Hi,`,
    ``,
    `Gumbo Cherries Indoor just dropped — sweet bubble-gum nose, cherry-front terps, indoor cure with 10/10 trichome density. Cleanest Gumbo pheno we've seen this year.`,
    ``,
    `SUBSCRIBERS ONLY — 15% OFF`,
    `Use code CHERRIES15 at checkout. Valid 72 hours.`,
    ``,
    `Shipped USA, all-in: ${live} (was ${slashed}, before code)`,
    ``,
    `Why this one:`,
    `✓ Distinctive cherry-bubblegum flavor (not generic)`,
    `✓ 10/10 indoor cure — frosted bag-appeal`,
    `✓ Discreet plain-label shipping nationwide`,
    ``,
    `These move in 24-48 hours. Lock yours in:`,
    `${url}`,
    ``,
    `Code CHERRIES15 expires in 72 hours.`,
    ``,
...
async function main() {
  ...
  const subject = `Gumbo Cherries just dropped — 15% off (subscribers only)`;
  ...
  console.log(`\n🚀 Sending Gumbo Cherries Indoor campaign to ${subscribers.length} active subscribers`);
  console.log(`   From: ${fromAddress}`);
  console.log(`   Subject: ${subject}`);
  ...
  try {
    await prisma.campaign.upsert({
      ...
      create: {
        slug: CAMPAIGN_SLUG,
        name: "Gumbo Cherries Indoor — Newsletter Drop (CHERRIES15)",
